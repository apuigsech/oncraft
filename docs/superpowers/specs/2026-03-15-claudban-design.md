...
When loading `.claudban/config.yaml`, ClaudBan validates:
...
3. If exists:
   - If the session is Idle (no running process), ClaudBan auto-resumes it with `claude --resume <session-id> --output-format stream-json` first
...
- If the `claude` binary is not found (not in PATH and not configured in global settings), ClaudBan shows an error prompting the user to install Claude Code or configure the binary path in settings
...
**State transitions:**
- **Active -> Idle**: The Claude Code process exits (naturally after completing a response, or unexpectedly via crash/signal). ClaudBan detects process exit and updates the card indicator.
- **Idle -> Active**: User sends a message in the chat panel, or a pipeline triggers on card move. ClaudBan spawns `claude --resume <session-id> --output-format stream-json`.
...
- User types in input, ClaudBan sends the text via stdin to the Claude Code process using the `stream-json` input protocol (newline-delimited JSON messages)
...
## Data Model (SQLite: `~/.claudban/claudban.db`)
